CCCC This reads input files and initializes variables
...
CCC Originally developed in May 2016
     ...
     & irealsize, ruletype, ictilde, icrule, nstep, nprint, ipopulation) 
      ...
      dimension ipopulation(1000000), index(100,3)
      dimension neighlist(1000000, 30), irule(2,30)
      namelist /files/ seedfile, rulefile
      namelist /lattice/ idimension, latticetype, 
     &   neighbourhood, isize
      namelist /steps/ nstep, nprint

CCC Set some default values: 3 dimensions, simple cubic lattice, Moore's neighbourhood 
      idimension = 3
      latticetype ='sc'
      neighbourhood = 'vonneumann'
      neighlist(:,:)=0
      isize = 100 
cc      itotsize = isize**idimension    
      ipopulation(:)=0
      nstep = 10
      nprint = 10

CCCC Read main input, with information on lattice, rule, number of iterations, output options 
      ...
      if(latticetype.ne.'sc'.and.latticetype.ne.'fcc') 
     &    STOP 'Wrong lattice'
      if(neighbourhood.ne.'vonneumann'.and.
     &   neighbourhood.ne.'moore')
     &    STOP 'Wrong neighbourhood'
...
cccc             write(*,*)'------------------'
cccc           enddo
cc      elseif(latticetype.eq.'fcc'.and.neighbourhood.eq.'vonneumann') then
cc             irealsize=isize**idimension*WHAT?? 
      else
         STOP 'No neighbours for this lattice-neighbourhood'
      endif

CCC Read and initialize the seed
      ...
      if(latticetypes.ne.latticetype)
     &    STOP 'Seed lattice different from input lattice'
      if(seedtype.eq.'indices') then
         do i =1, nseed
           read(12,*) index(i,1), index(i,2), index(i,3)
         enddo
CCC Now use this to initialize the array...
      ...
      close(12)
CCC End of seed initialization
...
CCC Read the rule
      ...
      if(latticetyper.ne.latticetype)
     &    STOP 'Lattice from rulefile different from input lattice' 
      if(ruletype.eq.'outer') then 
         read(12,*) ictilde
      elseif(ruletype.eq.'outer2') then
CC irule(1,:): when the central site is empty
         read(12,*) irule(1,0:nneigh)
CC irule(2,:): when the central site is full
         read(12,*) irule(2,0:nneigh)
         ictilde = 0
CCC This is the definition of C tilde from Packard and Wolfram, J Stat Phys 38, 901 (1985))
         do i=0,nneigh
